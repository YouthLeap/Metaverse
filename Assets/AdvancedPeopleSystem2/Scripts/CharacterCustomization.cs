...
namespace AdvancedPeopleSystem
{
    ...
    public class CharacterCustomization : MonoBehaviour, IPunObservable
    {
        ...
        public int Hatint = -1;
        public int Shirtint = -1;
        public int Pantsint = -1;
        public int Shoesint = -1;
        public int Accessoryint = -1;
        public int Hairint = -1;
        public int Beardint = -1;
        public int Item1int = -1;
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)
            {
                ...
                stream.SendNext(heightValue);
                stream.SendNext(headSizeValue);

                stream.SendNext(fat);
                stream.SendNext(breastsize);
                stream.SendNext(slimness);
                stream.SendNext(headOffset);

                stream.SendNext(new Vector3(Skin.r, Skin.g,Skin.b));
                stream.SendNext(new Vector3(Eye.r, Eye.g, Eye.b));
                stream.SendNext(new Vector3(Hair.r, Hair.g, Hair.b));
                stream.SendNext(new Vector3(Underpants.r, Underpants.g, Underpants.b));
                stream.SendNext(new Vector3(OralCavity.r, OralCavity.g, OralCavity.b));
                stream.SendNext(new Vector3(Teeth.r, Teeth.g, Teeth.b));

                stream.SendNext(Hatint);
                stream.SendNext(Shirtint);
                stream.SendNext(Pantsint);
                stream.SendNext(Shoesint);
                stream.SendNext(Accessoryint);
                stream.SendNext(Hairint);
                stream.SendNext(Beardint);
                stream.SendNext(Item1int);

            }
            else if (stream.IsReading)
            {

                heightValue=(float)stream.ReceiveNext();
                headSizeValue=(float)stream.ReceiveNext();

                SetHeadSize(headSizeValue);
                SetHeight(heightValue);

                fat = (float)stream.ReceiveNext();
                breastsize = (float)stream.ReceiveNext();
                slimness = (float)stream.ReceiveNext();
                headOffset = (float)stream.ReceiveNext();

                SetBlendshapeValue(false, CharacterBlendShapeType.Fat, fat);
                SetBlendshapeValue(false, CharacterBlendShapeType.Head_Offset, headOffset);

                if (selectedsettings.name == "FemaleSettings")
                {

                    SetBlendshapeValue(false, CharacterBlendShapeType.BreastSize, breastsize, new string[] { "Chest", "Stomach", "Head" },
                    new CharacterElementType[] { CharacterElementType.Shirt });
                    SetBlendshapeValue(false, CharacterBlendShapeType.Slimness, slimness);
                }
                else
                {
                    SetBlendshapeValue(false, CharacterBlendShapeType.Muscles, breastsize);
                    SetBlendshapeValue(false, CharacterBlendShapeType.Thin, slimness);
                }



                Vector3 temp = (Vector3)stream.ReceiveNext();
                Skin = new Color(temp.x,temp.y,temp.z);
                temp = (Vector3)stream.ReceiveNext();
                Eye = new Color(temp.x, temp.y, temp.z);
                temp = (Vector3)stream.ReceiveNext();
                Hair = new Color(temp.x, temp.y, temp.z);
                temp = (Vector3)stream.ReceiveNext();
                Underpants = new Color(temp.x, temp.y, temp.z);
                temp = (Vector3)stream.ReceiveNext();
                OralCavity = new Color(temp.x, temp.y, temp.z);
                temp = (Vector3)stream.ReceiveNext();
                Teeth = new Color(temp.x, temp.y, temp.z);
                ...
                Hatint = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Hat,Hatint);
                Shirtint = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Shirt, Shirtint);
                Pantsint = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Pants, Pantsint);
                Shoesint = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Shoes, Shoesint);
                Accessoryint = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Accessory, Accessoryint);
                Hairint = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Hair, Hairint);
                Beardint = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Beard, Beardint);
                Item1int = (int)stream.ReceiveNext();
                SetElementByIndex(CharacterElementType.Item1, Item1int);
            }
        }
        ...
        public Dictionary<CharacterBlendShapeType, float> caractarBlendShapevalue = new Dictionary<CharacterBlendShapeType, float>();
        ...
        private void Start()
        {
            ...
            {
                SetElementByIndex(CharacterElementType.Accessory, GetElementByIndex(CharacterElementType.Accessory));
                
                SetElementByIndex(CharacterElementType.Beard, GetElementByIndex(CharacterElementType.Beard));
                SetElementByIndex(CharacterElementType.Hair, GetElementByIndex(CharacterElementType.Hair));
                SetElementByIndex(CharacterElementType.Hat, GetElementByIndex(CharacterElementType.Hat));
                SetElementByIndex(CharacterElementType.Item1, GetElementByIndex(CharacterElementType.Item1));
                SetElementByIndex(CharacterElementType.Pants, GetElementByIndex(CharacterElementType.Pants));
                SetElementByIndex(CharacterElementType.Shirt, GetElementByIndex(CharacterElementType.Shirt));
                SetElementByIndex(CharacterElementType.Shoes, GetElementByIndex(CharacterElementType.Shoes));

                SetBodyColor(BodyColorPart.Eye,GetBodyColor(BodyColorPart.Eye));
                SetBodyColor(BodyColorPart.Hair, GetBodyColor(BodyColorPart.Hair));
                SetBodyColor(BodyColorPart.OralCavity, GetBodyColor(BodyColorPart.OralCavity));
                SetBodyColor(BodyColorPart.Skin, GetBodyColor(BodyColorPart.Skin));
                SetBodyColor(BodyColorPart.Teeth, GetBodyColor(BodyColorPart.Teeth));
                SetBodyColor(BodyColorPart.Underpants, GetBodyColor(BodyColorPart.Underpants));

                headSizeValue =  selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsheadSizeValue) : float.Parse(ServerManager.instance.character.FemaleSettingsheadSizeValue);
                heightValue = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsheightValue) : float.Parse(ServerManager.instance.character.FemaleSettingsheightValue);
                SetHeadSize(headSizeValue);
                SetHeight(heightValue);
                ...
                fat = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsFat) : float.Parse(ServerManager.instance.character.FemaleSettingsFat);
                SetBlendshapeValue(false, CharacterBlendShapeType.Fat, fat);
                headOffset = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsHead_Offset) : float.Parse(ServerManager.instance.character.FemaleSettingsHead_Offset);
                Debug.Log(selectedsettings.name+ CharacterBlendShapeType.Head_Offset.ToString());
                if (selectedsettings.name== "FemaleSettings")
                {
                    breastsize = float.Parse(ServerManager.instance.character.FemaleSettingsBreastSize);
                    SetBlendshapeValue(false, CharacterBlendShapeType.BreastSize, breastsize, new string[] { "Chest", "Stomach", "Head" },
                    new CharacterElementType[] { CharacterElementType.Shirt });
                    slimness = float.Parse(ServerManager.instance.character.FemaleSettingsSlimness);
                    SetBlendshapeValue(false, CharacterBlendShapeType.Slimness, slimness);
                    
                }
                else
                {
                    breastsize = float.Parse(ServerManager.instance.character.MaleSettingsMuscles);
                    SetBlendshapeValue(false, CharacterBlendShapeType.Muscles, breastsize);
                    slimness = float.Parse(ServerManager.instance.character.MaleSettingsThin);
                    SetBlendshapeValue(false, CharacterBlendShapeType.Thin, slimness);
                    
                }
                
                //Debug.Log(selectedsettings.name + CharacterBlendShapeType.Fat.ToString());
            }
        }
            ...
            if (currentBlendshapeAnimation != null)
            {
                ...
                for (var i = 0; i < currentBlendshapeAnimation.preset.blendshapes.Count; i++)
                {
                    if (currentBlendshapeAnimation.preset.UseGlobalBlendCurve)
                        SetBlendshapeValue(false, currentBlendshapeAnimation.preset.blendshapes[i].BlendType, currentBlendshapeAnimation.preset.blendshapes[i].BlendValue * currentBlendshapeAnimation.preset.weightPower * currentBlendshapeAnimation.preset.GlobalBlendAnimationCurve.Evaluate(currentBlendshapeAnimation.timer));
                    else
                        SetBlendshapeValue(false, currentBlendshapeAnimation.preset.blendshapes[i].BlendType, currentBlendshapeAnimation.preset.blendshapes[i].BlendValue * currentBlendshapeAnimation.preset.weightPower * currentBlendshapeAnimation.preset.blendshapes[i].BlendAnimationCurve.Evaluate(currentBlendshapeAnimation.timer));
                }
        ...
        public void SetBlendshapeValue(bool fromeditor,CharacterBlendShapeType type, float weight, string[] forPart = null, CharacterElementType[] forClothPart = null)
        {
            var typeName = type.ToString();
            if(SceneManager.GetActiveScene().name== "PlayerSelectionScene" && fromeditor)
            {
                //PlayerPrefs.SetFloat(selectedsettings.name + type.ToString(), weight);
                ServerManager.instance.character.UpdateCharacter(selectedsettings, type, weight.ToString());
            }
                
            Debug.Log(selectedsettings.name + type.ToString()+weight);
            //Debug.Log(PlayerPrefs.GetFloat(selectedsettings.name + type.ToString(), 0));
        ...
        public int GetElementByIndex(CharacterElementType type)
        {
            int result = -1;
            switch (type)
            {
                case CharacterElementType.Accessory:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsAccessory) : int.Parse(ServerManager.instance.character.FemaleSettingsAccessory);
                    break;
                case CharacterElementType.Beard:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsBeard) : int.Parse(ServerManager.instance.character.FemaleSettingsBeard);
                    break;
                case CharacterElementType.Hair:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsHair) : int.Parse(ServerManager.instance.character.FemaleSettingsHair);
                    break;
                case CharacterElementType.Hat:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsHat) : int.Parse(ServerManager.instance.character.FemaleSettingsHat);
                    break;
                case CharacterElementType.Pants:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsPants) : int.Parse(ServerManager.instance.character.FemaleSettingsPants);
                    break;
                case CharacterElementType.Shirt:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsShirt) : int.Parse(ServerManager.instance.character.FemaleSettingsShirt);
                    break;
                case CharacterElementType.Shoes:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsShoes) : int.Parse(ServerManager.instance.character.FemaleSettingsShoes);
                    break;
                case CharacterElementType.Item1:
                    result = selectedsettings.name == "MaleSettings" ? int.Parse(ServerManager.instance.character.MaleSettingsItem1) : int.Parse(ServerManager.instance.character.FemaleSettingsItem1);
                    break;
                default:
                    break;
        ...
        public void SetElementByIndex(CharacterElementType type, int index)
        {
            ...
            if(type== CharacterElementType.Hat)
                Hatint = index;
            if (type == CharacterElementType.Shirt)
                Shirtint = index;
            if (type == CharacterElementType.Pants)
                Pantsint = index;
            if (type == CharacterElementType.Shoes)
                Shoesint = index;
            if (type == CharacterElementType.Accessory)
                Accessoryint = index;
            if (type == CharacterElementType.Hair)
                Hairint = index;
            if (type == CharacterElementType.Beard)
                Beardint = index;
            if (type == CharacterElementType.Item1)
                Item1int = index;


            if(view!=null && view.IsMine)
            {
                //PlayerPrefs.SetInt(selectedsettings.name + type.ToString(), index);
                ServerManager.instance.character.UpdateCharacter(selectedsettings, type, index.ToString());
            }
            ...
            if (newPreset != null)
            {
                ...
                if (type == CharacterElementType.Shirt)
                {
                    ...
                    if (characterSelectedElements.GetSelectedIndex(CharacterElementType.Item1) != -1)
                        SetBlendshapeValue(false, CharacterBlendShapeType.BackpackOffset, 100);
                }
            ...
            else
            {
                if (type == CharacterElementType.Shirt)
                {
                    ...
                    if (characterSelectedElements.GetSelectedIndex(CharacterElementType.Item1) != -1)
                        SetBlendshapeValue(false, CharacterBlendShapeType.BackpackOffset, 0);
                }
                if (index != -1)
                {
        ...
        {
            heightValue = height;
            //PlayerPrefs.SetFloat(selectedsettings.name + "heightValue", heightValue);
            ServerManager.instance.character.UpdateCharacterWithHeight(selectedsettings, heightValue.ToString());
            if (originHip != null)
        ...
        {
            headSizeValue = size;
            //PlayerPrefs.SetFloat(selectedsettings.name+ "headSizeValue", headSizeValue);
            ServerManager.instance.character.UpdateCharacterWithHeadSize(selectedsettings, headSizeValue.ToString());
            if (headHip != null)
        ...
        private void SetHairByIndex(int index)
        {
            ...
            if (index != -1)
            {
                ...
                {
                    Debug.Log(string.Format("Hair with index {0} not found", index));
                    return;
                }
                for (var i = 0; i < (MaxLODLevels - MinLODLevels) + 1; i++)
                {
        ...
        public void SetBodyColor(BodyColorPart bodyColorPart, Color color)
        {
            ...
            ServerManager.instance.character.UpdateCharacter(selectedsettings, bodyColorPart, color);
        ...
        public Color GetBodyColor(BodyColorPart bodyColorPart)
        {
            ...
            {
                case BodyColorPart.Skin:
                    float r = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsSkinr) : float.Parse(ServerManager.instance.character.FemaleSettingsSkinr);
                    float g = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsSking) : float.Parse(ServerManager.instance.character.FemaleSettingsSking);
                    float b = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsSkinb) : float.Parse(ServerManager.instance.character.FemaleSettingsSkinb);
                    if (r != -1 && g != -1 && b != -1)
                        Skin = new Color(r,g,b);
                    return Skin;
                case BodyColorPart.Eye:
                    r = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsEyer) : float.Parse(ServerManager.instance.character.FemaleSettingsEyer);
                    g = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsEyeg) : float.Parse(ServerManager.instance.character.FemaleSettingsEyeg);
                    b = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsEyer) : float.Parse(ServerManager.instance.character.FemaleSettingsEyer);
                    if (r != -1 && g != -1 && b != -1)
                        Eye = new Color(r, g, b);
                    return Eye;
                case BodyColorPart.Hair:
                    r = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsHairr) : float.Parse(ServerManager.instance.character.FemaleSettingsHairr);
                    g = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsHairg) : float.Parse(ServerManager.instance.character.FemaleSettingsHairg);
                    b = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsHairb) : float.Parse(ServerManager.instance.character.FemaleSettingsHairb);
                    if (r != -1 && g != -1 && b != -1)
                        Hair = new Color(r, g, b);
                    return Hair;
                case BodyColorPart.Underpants:
                    r = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsUnderpantsr) : float.Parse(ServerManager.instance.character.FemaleSettingsUnderpantsr);
                    g = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsUnderpantsg) : float.Parse(ServerManager.instance.character.FemaleSettingsUnderpantsg);
                    b = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsUnderpantsb) : float.Parse(ServerManager.instance.character.FemaleSettingsUnderpantsb);
                    if (r != -1 && g != -1 && b != -1)
                        Underpants = new Color(r, g, b);
                    return Underpants;
                case BodyColorPart.OralCavity:
                    r = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsOralCavityr) : float.Parse(ServerManager.instance.character.FemaleSettingsOralCavityr);
                    g = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsOralCavityg) : float.Parse(ServerManager.instance.character.FemaleSettingsOralCavityg);
                    b = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsOralCavityb) : float.Parse(ServerManager.instance.character.FemaleSettingsOralCavityb);
                    if (r != -1 && g != -1 && b != -1)
                        OralCavity = new Color(r, g, b);
                    return OralCavity;
                case BodyColorPart.Teeth:
                    r = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsTeethr) : float.Parse(ServerManager.instance.character.FemaleSettingsTeethr);
                    g = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsTeethg) : float.Parse(ServerManager.instance.character.FemaleSettingsTeethg);
                    b = selectedsettings.name == "MaleSettings" ? float.Parse(ServerManager.instance.character.MaleSettingsTeethb) : float.Parse(ServerManager.instance.character.FemaleSettingsTeethb);
                    if (r != -1 && g != -1 && b != -1)
                        Teeth = new Color(r, g, b);
                    return Teeth;
            }
            return Color.clear;
        }
        ...
        public void RecalculateShapes()
        {
            foreach(var blendshape in characterBlendshapeDatas)
            {
                SetBlendshapeValue(false, blendshape.type, blendshape.value);
            }
        }
        ...
        public void StopBlendshapeAnimations()
        {
            if (currentBlendshapeAnimation != null)
            {
                for (var i = 0; i < currentBlendshapeAnimation.preset.blendshapes.Count; i++)
                {
                    SetBlendshapeValue(false, currentBlendshapeAnimation.preset.blendshapes[i].BlendType, currentBlendshapeAnimation.blendShapesTemp[i].BlendValue);
                }
            }
        }
        ...
        public void ResetAll(bool ignore_settingsDefaultElements = true)
        {
            ...
            foreach (var blendshape in characterBlendshapeDatas)
                SetBlendshapeValue(false, blendshape.type, 0);
