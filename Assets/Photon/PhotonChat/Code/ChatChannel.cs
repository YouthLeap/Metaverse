...
namespace Photon.Chat
{
    ...
    public class ChatChannel
    {
        ...
        public string ToStringMessages()
        {
            ...
            {
                if(((Dictionary<string, object>)this.Messages[i]).TryGetValue("msg",out object msg))
                    txt.AppendLine(string.Format("{0}: {1}", this.Senders[i], msg));
            }
            return txt.ToString();
        }

        public string ToStringLastMessages(string name)
        {
            StringBuilder txt = new StringBuilder();
            for (int i = this.Messages.Count-1; i >0; i--)
            {
                if (((Dictionary<string, object>)this.Messages[i]).TryGetValue("msg", out object msg) && this.Senders[i]==name)
                {
                    txt.AppendLine(string.Format("{0}: {1}", this.Senders[i], msg));
                    break;
                }
