...
namespace RGSK
{
    public class RaceUI : MonoBehaviour
    {
		...
		public GameObject m_EddyRaceManager;
		public GameObject[] ArraySpeedImage;
        ...
        {
            if (!EddyRaceManager.instance) return;

            if (driftUI.driftPanel) driftUI.driftPanel.SetActive(EddyRaceManager.instance._raceType == EddyRaceManager.RaceType.Drift);
            if (raceResultsPanel) raceResultsPanel.SetActive(EddyRaceManager.instance._raceType != EddyRaceManager.RaceType.Drift);
            if (driftResultsPanel) driftResultsPanel.SetActive(EddyRaceManager.instance._raceType == EddyRaceManager.RaceType.Drift);

            if (EddyRaceManager.instance._raceType == EddyRaceManager.RaceType.Drift)
            {
                if (driftUI.goldPoints) driftUI.goldPoints.text = EddyRaceManager.instance.goldDriftPoints.ToString("N0");
                if (driftUI.silverPoints) driftUI.silverPoints.text = EddyRaceManager.instance.silverDriftPoints.ToString("N0");
                if (driftUI.bronzePoints) driftUI.bronzePoints.text = EddyRaceManager.instance.bronzeDriftPoints.ToString("N0");
            }
        }
        ...
        void UpdateUI()
        {
            ...
            switch (EddyRaceManager.instance._raceType)
            {

                case EddyRaceManager.RaceType.Circuit:
                    DefaultUI();
                    break;

                /*case EddyRaceManager.RaceType.Sprint:
                    DefaultUI();
                    break;
                */

                case EddyRaceManager.RaceType.LapKnockout:
                    DefaultUI();
                    break;

                case EddyRaceManager.RaceType.TimeTrial:
                    TimeTrialUI();
                    break;

                case EddyRaceManager.RaceType.SpeedTrap:
                    DefaultUI();
                    break;

                case EddyRaceManager.RaceType.Checkpoints:
                    CheckpointRaceUI();
                    break;

                case EddyRaceManager.RaceType.Elimination:
                    EliminationRaceUI();
                    break;

                case EddyRaceManager.RaceType.Drift:
                    DriftRaceUI();
                    break;
            }

            switch (EddyRaceManager.instance._raceState)
            {
                case EddyRaceManager.RaceState.StartingGrid:
                    ShowStartingGrid();
                    break;

                case EddyRaceManager.RaceState.Racing:
                    ShowInRaceStandings();
                    WrongwayUI();
                    break;

                case EddyRaceManager.RaceState.Complete:
                    if (EddyRaceManager.instance._raceType != EddyRaceManager.RaceType.Drift)
                    {
                        ShowRaceResults();
                    }
                ...
                case EddyRaceManager.RaceState.Replay:
                    ShowReplayUI();
                    break;
            }
        }
        ...
        void DefaultUI()
        {
            ...
            //LAP
            if (racingUI.lap)
                racingUI.lap.text = "Lap : " + player.lap + "/" + EddyRaceManager.instance.totalLaps;
        ...
        void CheckpointRaceUI()
        {
            ...
            //CHECKPOINTS
            if (racingUI.lap)
                racingUI.lap.text = "CP " + player.checkpoint + "/" + player.checkpoints.Count * EddyRaceManager.instance.totalLaps;
        ...
        void EliminationRaceUI()
        {
            ...
            //TIMER
            if (racingUI.currentLapTime)
                racingUI.currentLapTime.text = "Time : " + EddyRaceManager.instance.FormatTime(EddyRaceManager.instance.eliminationCounter);
        ...
        void VehicleGUI()
        {
            ...
            if (vehicleUI.currentSpeed)
            {
				...
				ArraySpeedImage [m_CurrentVal].SetActive (false);
				int val =Mathf.FloorToInt( player.GetComponent<Car_Controller>().currentSpeed / 4.8f);
				if(val > 3){
					m_Light.SetActive (false);
				}
				ArraySpeedImage [val].SetActive (true);
				m_CurrentVal = val;
...
//				Debug.Log (fraction.ToString());
        ...
        public void UpdateUIPanels()
        {
            ...
            switch (EddyRaceManager.instance._raceState)
            {
                ...
                case EddyRaceManager.RaceState.Paused:
                    if (startingGridPanel) startingGridPanel.SetActive(false);
                ...
                case EddyRaceManager.RaceState.KnockedOut:
                    if (startingGridPanel) startingGridPanel.SetActive(false);
        ...
        void ShowInRaceStandings()
        {
            ...
            for (int i = 0; i < RankManager.instance.totalRacers; i++)
            {
                if (i < racingUI.inRaceStandings.Count)
                {
                    ...
                    //Name
                    if (racingUI.inRaceStandings[i].name) racingUI.inRaceStandings[i].name.text = (EddyRaceManager.instance._raceType != EddyRaceManager.RaceType.SpeedTrap) ? _statistics.racerDetails.racerName
        ...
        {
			m_EddyRaceManager.GetComponent<EddyRaceManager> ().DataRoad ();
            StartCoroutine(EddyRaceManager.instance.Countdown(time));
        }

        public void PauseResume()
        {
            EddyRaceManager.instance.PauseRace();
        }

        public void Restart()
        {
            //unpause inorder to reset timescale & audiolistener vol
            if (EddyRaceManager.instance._raceState == EddyRaceManager.RaceState.Paused)
            {
                PauseResume();
            }
