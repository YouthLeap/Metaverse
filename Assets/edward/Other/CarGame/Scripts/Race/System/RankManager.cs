        ...
        void SetCarRank()
        {
            for (int r = 0; r < currentRacers; r++)
            {
                ...
                else
                {
                    ...
                    if (racerRanks[r].racer && racerRanks[r].racer.GetComponent<Statistics>())
                    {
                        if (totalPoints >= EddyRaceManager.instance.goldDriftPoints)
                            racerRanks[r].racer.GetComponent<Statistics>().rank = 1;

                        if (totalPoints < EddyRaceManager.instance.goldDriftPoints && totalPoints >= EddyRaceManager.instance.silverDriftPoints)
                            racerRanks[r].racer.GetComponent<Statistics>().rank = 2;

                        if (totalPoints < EddyRaceManager.instance.silverDriftPoints && totalPoints >= EddyRaceManager.instance.bronzeDriftPoints)
                            racerRanks[r].racer.GetComponent<Statistics>().rank = 3;

                        if (totalPoints < EddyRaceManager.instance.bronzeDriftPoints)
