...
namespace RGSK
{
    public class OpponentControl : MonoBehaviour
    {
        ...
        void CheckIfStuck()
        {
            if (!EddyRaceManager.instance)
                return;

            //Add to the respawn timer incase the AI gets stuck
            if (EddyRaceManager.instance.raceStarted && !stats.finishedRace && !stats.knockedOut)
            {
        ...
        void RespawnRacer()
        {
            ...
            EddyRaceManager.instance.RespawnRacer(transform, stats.lastPassedNode, 3.0f);      
        }
